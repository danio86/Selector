...
no_go_words = ['Carrots', 'Shortbread', 'Porridge', 'Oyster', 'Doughnuts',
               ...
               'Cabbage', 'Grilled', 'Pizza', 'Cookies', 'Fauxnuts', 'Fries']
        ...
        try:
            ...
            if final_time < 30 and 'hour' not in final_choice[2]:
                ...
                for meat in final_choice_lst:
                    ...
                    meat_in_recipe.append(meat.lower())
                if food_type in ('v', 'V'):
    ...
    while True:
        ...
        try:
            if pre_choice in random_inputs:
                ...
                media_type = input('Enter m for Movie or s for Series: \n')
                while 1:
                    try:
                        if 'm' in media_type or 'M' in media_type:
                            while True:
                                ...
                                if 'Film' in final_choice:
                                    ...
                                    final_selection.update(
                                        {heading: data for heading,
                                         data in zip(keys[0], final_choice)})
                                    print('\nMore selective criteria?')
                                    answer_media_type = input(
                                        'Enter y for Yes or n for No: \n')
                                    if ('n' in answer_media_type or
                                       'N' in answer_media_type):
                                        return final_selection
                                    elif ('y' in answer_media_type or
                                          'Y' in answer_media_type):
                                        print('\nSelect a genre. \nEnter a'
                                              'for Action, c for Comedy, d for'
                                              'Drama, \nf for fantasy, h for'
                                              'Horror, k for Kids, \nm for'
                                              'Mystery, r for Romance, s for'
                                              'Sports \nor t for Thriller')
                                        while 1:
                                            try:
                                                genere_answer = input(
                                                    'Enter Genre: \n')
                                                if (genere_answer.lower()
                                                   in genre_lst):
                                                    # genre_lst is a global
                                                    # list with all possible
                                                    # options. If selection
                                                    # not in list, user has
                                                    # to try again.
                                                    movie = genre_selection(
                                                        media_type,
                                                        genere_answer.lower())
                                                    # calls genre_selection
                                                    # def with movie
                            ...
                            while True:
                                ...
                                if 'Series' in final_choice:
                                    final_selection.update(
                                        {heading: data for heading, data in
                                         zip(keys[0], final_choice)})
                                    answer_media_type = input(
                                        '\nMore selective criteria? \n'
                                        'Enter y for Yes or n for No: \n')
                            ...
                            raise ValueError(f'Invalid data: {media_type}!'
                                             'Please try again!\n')
                    except ValueError as value_error:
                        print(value_error)
                        media_type = input('\nHow much time you want to'
                                           'spend?\nEnter m for Movie or s for'
                                           'Series: \n')
                        continue
            else:
                raise ValueError(f'Invalid data: {pre_choice}!'
                                 'Please try again!\n')
        except ValueError as value_error:
            print(value_error)
    return True
            ...
            if pre_choice in random_inputs:
                ...
                while True:
                    if 'd' in food_type or 'D' in food_type:
                        loading_time()
                        final_choice = random.choice(clean_file(topic))
                        if ('Drink' in final_choice[2] or
                            'drink' in final_choice[2] or
                            'cocktail' in final_choice[2] or
                                'punch' in final_choice[2]):
                            # final_choice[2] is the preparation (str) of the
                            # ramdomly chosen recipe
                            final_selection.update({heading: data for heading,
                                                    data in zip(keys[0],
                                                                final_choice)})
                            no_drink = any(same in no_go_words for same in
                                           final_choice[0].split())
                            if not no_drink:
                                print('')
                                answer_food_type = input('\nMore selective '
                                                         ...
                                                         'for Yes or n for '
                                                         'No: \n')
                                if ('n' in answer_food_type or
                                   'N' in answer_food_type):
                                    return final_selection
                                elif ('y' in answer_food_type or
                                      'Y' in answer_food_type):
                                        ...
                                        try:
                                            ...
                                            if alcohol_answer in ('a', 'A'):
                                                for alc in final_choice_lst:
                                                    ...
                                                    # of all ingredients which
                                                    # are not letters
                                                    if (alc.lower() in alcohol
                                                       and 'Drink' in
                                                       final_choice[2] or
                                                       alc.lower() in alcohol
                                                       and 'drink' in
                                                       final_choice[2] or
                                                       alc.lower() in alcohol
                                                       and 'punch' in
                                                ...
                                                for alc in final_choice_lst:
                                                    ...
                                                    # puts all cleaned
                                                    # lowercase ingredients in
                                                    # list
                                                no_alc = True
                                                no_alc = any(same in alc_lst
                                                             for same in
                                                             alcohol)
                                                # checks if there is any same
                                                # word (match) in the two
                                                # lists. Returns True/False
                                                title = final_choice[0]
                                                for title_part in title:
                                                    title_part = title.split()
                                                no_go_alc_words = any(
                                                    no_go in no_go_words for
                                                    no_go in title_part)
                                                # checks whether the choice
                                                # that meets all criteria is
                                                # still wrong
                                                if (
                                                    not no_alc and 'Drink' in
                                                    final_choice[2] or not
                                                    no_alc and 'drink' in
                                                    final_choice[2] or not
                                                    no_alc and 'Refresher' in
                                                    final_choice[0] or
                                                        'Matcha Latte' in
                                                        final_choice[0]):
                                                    if (no_go_alc_words is
                                                            False):
                                                        return final_selection
                                            else:
                                                raise ValueError(
                                            ...
                                            alcohol_answer = input(
                                                ...
                                                ' \nEnter a for Alcohol or n '
                                                'for Non Alcoholic: \n')
                                            continue
                                else:
                                    print(f'Invalid data: {answer_food_type}! '
                                          'Please try again!\n')
                        ...
                        if 'n' in answer_food_type or 'N' in answer_food_type:
                            return final_selection
                        elif ('y' in answer_food_type or 'Y' in
                              answer_food_type):
                            vegy_answer = input('\nAre you vegetarian? \nEnter'
                                                ' v for I am Vegy or m for I '
                                                'want Meat: \n')
                            while 1:
                                try:
                                    final_choice = random.choice(clean_file
                                                                 (topic))
                                    final_selection.update({heading: data for
                                                            heading, data in
                                                            zip(keys[0],
                                                                final_choice)})
                                    if vegy_answer in ('m', 'M'):
                                        ...
                                        if not no_meat:
                                            try:
                                                answer_time = input(
                                                    ...
                                                    "to spend (cutting/mixing "
                                                    "excluded)?\nEnter i for I"
                                                    " don't care or l for less"
                                                    " than 30 Minutes: \n")
                                        ...
                                        raise ValueError(
                                            f'Invalid data: {vegy_answer}! '
                                            'Please try again!\n')
                                except ValueError as value_error:
                                    print(value_error)
                                    vegy_answer = input(
                                        '\nAre you vegetarian? \nEnter v for I'
                                        ' am vegy or m for I want meat: \n')
                                    continue
                        else:
                            print(f'Invalid data: {answer_food_type}! Please'
                                  ' try again!\n')
                    else:
                        print(f'Invalid data: {food_type}! Please try '
                              'again!\n')
                        food_type = input('\nDo you want to eat or to drink? '
                                          '\nEnter e for Eat or d for '
                                          'Drink: \n')
 ...
 |____/|_____|_____|_____\____| |_| \___/|_| \_\
    ...
    try:
        ...
        if happy_user in ('n', 'N'):
            ...
            print('\nHave a good time!\n')
            return clean_final_selection
        else:
            raise ValueError(f'Invalid data: {happy_user}!'
                             'Please try again!\n')
    except ValueError as value_error:
        print(value_error)
